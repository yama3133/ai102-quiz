...
import { questions as originalQuestions, CATEGORIES } from './questions';

const clean = (s) => s.replace(/^[A-Z]\.\s*/, '');
...
function buildExamSet() {
  ...
  // Largest-remainder method: floor each share, then distribute leftover seats
  // to the categories with the biggest fractional parts. Guarantees the total
  // equals EXAM_TOTAL even when raw shares are like 10.5, 4.5, etc.
  const raw = CATEGORIES.map(c => ({
    name: c.name,
    exact: (c.weight / totalWeight) * EXAM_TOTAL,
  }));
  const allocated = raw.map(r => ({ name: r.name, count: Math.floor(r.exact), frac: r.exact - Math.floor(r.exact) }));
  const remaining = EXAM_TOTAL - allocated.reduce((s, a) => s + a.count, 0);
  const order = [...allocated].sort((a, b) => b.frac - a.frac);
  for (let i = 0; i < remaining; i++) {
    const target = allocated.find(a => a.name === order[i].name);
    target.count += 1;
  }
  const result = [];
  for (const a of allocated) {
    const pool = originalQuestions.filter(q => q.category === a.name);
    result.push(...shuffleArray(pool).slice(0, a.count));
  }
...
const App = () => {
  ...
  const startQuiz = (mode, options = {}) => {
    try {
      ...
      if (mode === 'ordered') {
        ...
        qList = shuffleArray(originalQuestions.filter(q => q.category === options.category));
      } else if (mode === 'exam') {
        qList = buildExamSet();
      } else {
        qList = [...originalQuestions];
  ...
  if (stage === 'title') {
    return <TitleScreen total={originalQuestions.length} onSelectMode={(m) => {
      if (m === 'category') {
        setStage('category');
      } else {
        startQuiz(m);
  ...
  if (stage === 'category') {
    return <CategoryScreen onPick={(cat) => startQuiz('category', { category: cat })} onBack={resetQuiz} />;
  }

  if (isQuizFinished) {
    return <ResultScreen score={score} total={questions.length} onBack={resetQuiz} mode={quizMode} />;
  }
...
function modeLabel(mode) {
  if (!mode) return '';
  switch (mode.mode) {
    case 'ordered':  return '全問順番';
    case 'random':   return '全問ランダム';
    case 'category': return `カテゴリー · ${mode.category}`;
    case 'exam':     return '本番形式 60問';
    default: return '';
  }
}

function TitleScreen({ total, onSelectMode }) {
  return (
    <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
      <Ambient />
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 720, width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>
        <div className="chip fade-in">
          <span className="chip-dot" />
          <span>Microsoft AI-102 · Azure AI Engineer</span>
        </div>
        ...
        <div className="panel slide-up" style={{ animationDelay: '0.05s', padding: '20px 28px', display: 'flex', alignItems: 'center', gap: 28, flexWrap: 'wrap', justifyContent: 'center' }}>
          <Stat label="QUESTIONS" value={String(total)} color="var(--azure)" />
          <Divider />
          <Stat label="CATEGORIES" value={String(CATEGORIES.length)} color="var(--cyan)" />
          <Divider />
          <Stat label="EXAM" value={`${EXAM_TOTAL} Q`} color="var(--violet)" />
        </div>
        ...
        <div className="panel slide-up" style={{ animationDelay: '0.1s', width: '100%', padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <ModeButton
            primary
            title="全問 順番通り"
            sub={`第1問 → 第${total}問の順で出題`}
            badge={`${total}問`}
            onClick={() => onSelectMode('ordered')}
          />
          <ModeButton
            title="全問 ランダム"
            sub="順番をシャッフルして出題"
        ...
        <div className="label fade-in-slow" style={{ marginTop: 4 }}>
          MOCK EXAM · MULTIPLE CHOICE · WITH EXPLANATIONS
        </div>
      </div>
    </div>
  );
...
function ModeButton({ title, sub, badge, primary, highlight, onClick }) {
  return (
    <button
      className={`mode-btn ${primary ? 'mode-btn-primary' : ''} ${highlight ? 'mode-btn-highlight' : ''}`}
      onClick={onClick}
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 720, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
        ...
        <div className="panel slide-up" style={{ width: '100%', padding: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {CATEGORIES.map((c) => (
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
                <span className="mode-btn-title">{c.name}</span>
                <span className="mode-btn-sub">公式配分 {c.weight}%</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
...
function QuizScreen({ question, currentIndex, total, score, selectedAnswers, showResult, onAnswer, onNext, onExit, mode }) {
          ...
          <div style={{ flex: 1, minWidth: 200, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{
              ...
              <div style={{
                height: '100%',
                width: `${progressPct}%`,
                background: 'linear-gradient(90deg, var(--cyan) 0%, var(--azure) 100%)',
          ...
          <section key={`q-${currentIndex}`} className="panel panel-hl slide-up" style={{ padding: '28px 28px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <span className="label mono" style={{ color: 'var(--azure)' }}>
...
function ResultScreen({ score, total, onBack, mode }) {
  const pct = total > 0 ? Math.round((score / total) * 100) : 0;
  const passed = pct >= 70;
  const accent = passed ? 'var(--success)' : 'var(--azure)';
  const accentRGB = passed ? '34, 197, 94' : '0, 120, 212';
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 520, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, textAlign: 'center' }}>
        ...
        {mode && (
          <div className="label slide-up">{modeLabel(mode)}</div>
        )}
        ...
        <div className="panel slide-up" style={{
          ...
          <div style={{
            ...
            <div style={{
              height: '100%',
              width: `${pct}%`,
              background: `linear-gradient(90deg, ${accent} 0%, ${passed ? 'var(--cyan)' : 'var(--azure-light)'} 100%)`,
